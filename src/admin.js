...
// Shared session config so AdminJS and /api use ONE session (see config/session.js)
import { sessionStore, sessionOptions, SESSION_COOKIE_NAME, SESSION_SECRET_VALUE } from './config/session.js';
...
const setupAdminPanel = async (app) => {
    ...
    try {
                    ...
                    options: {
                        ...
                        properties: {
                            ...
                            password: {
                                ...
                                components: { edit: Components.PasswordInput }
                            },
        ...
        // dist/.adminjs is emitted during the build step (see src/bundle.js).
        ...
        const authOptions = {
            authenticate,
            // MUST match the global session so AdminJS writes req.session.adminUser
            // into the SAME cookie/store that /api reads. cookiePassword is used by
            // AdminJS as the session secret; cookieName as the cookie name.
            cookiePassword: SESSION_SECRET_VALUE,
        ...
        const adminJsRouter = AdminJSExpress.buildAuthenticatedRouter(
            ...
            null,
            {
                // Reuse the exact same persisted store + cookie options as the
                // main app. secret/name are forced by authOptions above, but the
                // store and cookie flags must be passed here too.
                store: sessionStore,
                resave: false,
